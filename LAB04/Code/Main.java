...
public class Main {
    ...
    private static void executarTestesAutomatizados() {
        ...
        //testes de movimentacao do robo
        ...
        System.out.println(roboLimpador.getDescricao());


        // --- Testes ComunicadorCentral ---  VALE RESSALTAR QUE TEM ALGUNS CASOS QUE ESTÃO NO FINAL DOS TESTES DO BOMBEIRO
         System.out.print("---------------------------------------------------------------------------------------------------------------------------\n");
        ...
        try { roboBombeiro.apagar_fogo(comunicador); } catch (Exception e) { System.err.println("Teste [OK] ErrorApagarFogoException (Sem fogo perto): " + e.getMessage()); }
        try { ambiente.moverEntidade(roboBombeiro, 75, 5, 1); System.out.println("Teste [OK] Mover Bombeiro (Próximo do fogo).\n"); } catch (Exception e) { System.err.println("Teste [FALHA]: " + e.getMessage()); }        
...
//DEPOIS IREI VER O MENU INTERATIVO, MAS ENQUANTO ISSO VOU CONSERTAR O QUE VI ATÉ AGORA NOS CASOS DE TESTE
    ...
    private static void listarObstaculos() {
        ...
        System.out.println("Todas as posições dadas são dos cantos superiores esquerdos" +
        ...
        for (TipoObstaculo tipo : TipoObstaculo.values()) {
            ...
            if (!obsTipo.isEmpty()) {
                ...
                obsTipo.forEach(o -> System.out.printf(" - Posição: (%d, %d).\n", o.getX(), o.getY()));
                System.out.println("-----------------------------------");
            }
        }
